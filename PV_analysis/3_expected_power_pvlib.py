...
Usage:
  ...
  python 3_expected_power_pvlib.py
  ...
  python 3_expected_power_pvlib.py --weather raw --campus BUNDOORA

  # Custom output directory (default: PV_analysis/data_pvlib):
  python 3_expected_power_pvlib.py --building library --output-dir ../some_folder
"""
...
# Standalone CSV output (default); override with --output-dir
PVLIB_DATA_DIR = os.path.join(config.BASE, "data_pvlib")
...
def run(
    ...
    # ── save ──
    out_dir = os.path.abspath(output_dir) if output_dir else PVLIB_DATA_DIR
...
def main() -> None:
    ...
    ap.add_argument(
        ...
        default=None,
        metavar="DIR",
        help=f"Folder for CSV (default: {PVLIB_DATA_DIR})",
    )
    args = ap.parse_args()
